...
| A subagent pane says the visualization failed | Check the preview's status, allowed output directory, and whether that desktop host can render local visualizations. A pane error alone does not prove hook delivery or token attribution failed; inspect any saved child receipt or render the child Task through the CLI. |
...
| Invalid output directory | Use an absolute nonsymlink path under the Task's native visualization root or catalog workspace. A subagent can use its verified root Task's native visualization root. |
