...
| No update or reauthorization notice | Trust the new standalone reminder once in CLI `codex` → `/hooks`. Untrusted or disabled reminders cannot execute. Use `updates check --refresh` from the installed plugin for manual read-back. [Coverage and cache](UPDATE_NOTICES.md). |
| No automatic card | Confirm the plugin is installed, hooks are trusted, and this is a new Task. Run `auto-report status`. |
| Parent Task has a report, but a subagent has no own card or receipt | Check that the updated plugin is installed and every changed or untrusted definition, including `SubagentStart`, was reviewed and trusted in CLI `/hooks`; start a new Task and subagent. Confirm the host delivered the child's lifecycle events and native Task/turn IDs. A parent card or shared session ID does not establish a child baseline. |
| Subagent's report differs from the parent Task's descendant subtotal | Match the same hashed `@` selector in the parent descendant row and child card or report, then inspect coverage and time windows. The subagent's own native counter and receipts describe that Task; the parent subtotal uses separately verified native lineage and attributable usage for its interval. Missing or bounded lineage/counters remain partial or unavailable. |
| A subagent pane says the visualization failed | In 0.7.3, an inherited parent visualization directory is relocated to the child's own native root; a sandbox-denied relocation can fall back once to the child's workspace. The reference must point to that actual file. Start a new Task for the updated runtime; old snapshots are not rewritten. Other host rendering errors still need separate diagnosis. |
...
| The revised report differs from the inline card or original terminal report | A completion revision can include usage written after `Stop` or `SubagentStop`. Look for `revision: 2`, `reconciliation_status`, and `reconciled_at` in the revised JSON. Original terminal JSON, HTML, and Markdown receipts and inline snapshots are preserved. A fresh Task-report query selects the latest published revision. |
| No completion revision appears | Reconciliation runs once per reported `Stop` or `SubagentStop`, only while automatic reports are enabled. It checks at most eight times with a 25-second retry deadline and requires that exact Task and turn's native `task_complete` record. Missing completion, unavailable source data, or worker failure leaves the provisional terminal receipt intact. It does not keep retrying indefinitely. |
...
| The original Markdown link did not change | This is expected: reconciliation preserves all original terminal files and does not rewrite their links. Request a fresh Task report for the latest published revision, or inspect the separate `.reconciled.md` receipt. |
...
| Invalid output directory | Use an absolute nonsymlink path under the Task's native visualization root or catalog workspace. A verified child request under its root Task's native visualization directory is relocated before writing; unrelated roots, traversal and symlinks are rejected. |
